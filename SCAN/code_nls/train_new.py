...
if __name__ == '__main__':
    ...
    dataset_root1 ='../../DATASET/DATAD/0123'
    dataset_root2 = '../../DATASET/DATAD/45'
    tf_train,tf_test = greyscale_make_transforms(input_sz)
    img_train = ImagesDataset2(
        root=dataset_root1,
        transform=tf_train,
        train=True
    )
    ...
    Optimizer = optim.AdamW(model.parameters(), lr=Lr, betas=(0.6, 0.999), eps=1e-08, weight_decay=1e-3,
                            amsgrad=True)
    Explr = torch.optim.lr_scheduler.StepLR(Optimizer, step_size=10, gamma=0.9, last_epoch=-1)


    train(model,Criterion,Optimizer,Explr, num_epochs, dataloader_train, dataloader_test, device,'1121_datad0123_test45')
