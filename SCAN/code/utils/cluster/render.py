...
def vote_result(
    ...
    true_label = flat_targets_all.cpu().numpy()

    vote_mat = np.zeros([len(true_label), config.num_sub_heads], dtype=np.int32)

    for i in range(config.num_sub_heads):
        pre_soft = soft_predss_all[i].cpu().numpy()
        pre_label = np.argmax(adjust_matrix(pre_soft, all_matches[i]), axis=1)
        vote_mat[:, i] = pre_label
    vote_mat = vote_mat.astype(np.int32)

    vote_label = []
    for i in range(len(vote_mat)):
        vote_label.append(np.argmax(np.bincount(vote_mat[i])))
    vote_label = np.array(vote_label)

    acc = np.sum(vote_label == true_label) / len(true_label)
    print(
        "The Vote_count is {} and The {} sub_heads Vote label ACC = {:.4f}".format(
            vote_count, config.num_sub_heads, acc
        )
    )


def adjust_matrix(y_pred, match):
    h, w = y_pred.shape
...
def save_progress(
    ...
    mapping_assignment_dataloader and  mapping_test_dataloader
    index:save_progression_count
    sobel:False
    render_count: -1
    ...
    '''
  Here!!  new code  plot  Confusion matrix
  Start
  '''
    conf_out_dir = os.path.join(config.out_dir, "confusion_matrix")
    if not os.path.exists(conf_out_dir):
        os.makedirs(conf_out_dir)

    pre_label = flat_predss_all[best_sub_head].cpu().numpy()
    true_label = flat_targets_all.cpu().numpy()

    confuse = conf(true_label, pre_label, num_classes=config.gt_k)

    plot_confusion_matrix(
        confuse,
        classes=classes,
        conf_out_dir=conf_out_dir,
        model_ind=config.model_ind,
  ...
  '''
    ...
    soft_predss_all_numpy = soft_predss_all[best_sub_head].cpu().numpy()
    np.savetxt(os.path.join(config.out_dir, 'soft_pred_all.txt'), soft_predss_all_numpy)

    np.savetxt(os.path.join(config.out_dir, 'true_label.txt'), true_label)
    np.savetxt(os.path.join(config.out_dir, 'match.txt'), match)
    ...
    for colour_i, colours in enumerate(all_colours):
        ...
        point_half_side = 1  # size 2 * pixel_half_side + 1   = 3
...
def plot_confusion_matrix(
    ...
    plt.savefig(
        os.path.join(
            conf_out_dir,
            "%d_run_%d_pointcloud_%s_Confusion_matrix.png"
            % (model_ind, render_count, index),
        )
    )
    plt.clf()


def conf(y_true, y_pred, num_classes=10):
