...
from code.utils.cluster.render import save_progress,vote_result
from code.utils.cluster.mine_data import ImagesDataset,ImagesDataset_NSL

# Options ----------------------------------------------------------------------

parser = argparse.ArgumentParser()
parser.add_argument("--model_ind", type=int, required=True, default=117)
parser.add_argument("--arch", type=str, default="ClusterNet")
parser.add_argument("--opt", type=str, default="Adam")
parser.add_argument("--mode", type=str, default="IID")

parser.add_argument("--dataset", type=str, default="MINE_MNIST")
...
parser.add_argument("--out_root", type=str, default="outdir")
...
parser.add_argument("--Integrated", default=False, action="store_true")
...
parser.add_argument("--tf1_crop_sz", type=int, default=256)

parser.add_argument("--tf2_crop_szs", type=int, nargs="+", default=[256])  # allow diff crop for imgs_tf
...
parser.add_argument("--input_sz", type=int, default=256)
...
def train(render_count=-1):
  ...
  if config.Integrated:
    vote_count = 0
    vote_result(config,net,mapping_assignment_dataloader,
                mapping_test_dataloader,vote_count,sobel=False)
    vote_count+=1
  ...
  for e_i in range(next_epoch, config.num_epochs):
    ...
    for head_i in range(2):
      head = heads[head_i]  # head_i = 0,1   head = "B",A""
      if head == "A":
      ...
      for head_i_epoch in range(head_epochs[head]): # head_epochs = {A:1,B:1}
        sys.stdout.flush()

        iterators = (d for d in dataloaders)  #dataloaders = [tf1  tf2 tf2 tf2 tf2 tf2] 1 + 5 aux

        b_i = 0
        for tup in zip(*iterators):
          net.module.zero_grad()  # clear grad

          all_imgs = torch.zeros((config.batch_sz, config.in_channels,
                                  config.input_sz,
                                  config.input_sz)).cuda()   # all zeros 700 1 64 64 .to cuda
          all_imgs_tf = torch.zeros((config.batch_sz, config.in_channels,
                                     config.input_sz,
                                     config.input_sz)).cuda()   #all zeros 700 1 64 64 .to cuda

          imgs_curr = tup[0][0]  # always the first   # 700 1 64 64
          curr_batch_sz = imgs_curr.size(0)    # batchsize
          for d_i in range(config.num_dataloaders):  # num_dataloaders = 5  0 , 1 , 2, 3, 4
          ...
          x_outs = net(all_imgs)    # tf1 out   -》5 head
          x_tf_outs = net(all_imgs_tf)  # tf2 out -》5 head

          avg_loss_batch = None  # avg over the heads
          avg_loss_no_lamb_batch = None
          for i in range(config.num_sub_heads):  #num_sub_heads = 5 ,0 1 2 3 4
          ...
          if ((b_i % 100) == 0) and save_progression:
            save_progress(config, net, mapping_assignment_dataloader,
                          mapping_test_dataloader, save_progression_count,
                          sobel=False,
                          render_count=render_count)
    ...
    if is_best or (e_i % config.save_freq == 0):
      ...
      if is_best:
        ...
        with open(os.path.join(config.out_dir, "best_config.pickle"), 'wb') as outfile: pickle.dump(config, outfile)

        with open(os.path.join(config.out_dir, "best_config.txt"), "w") as text_file: text_file.write("%s" % config)

      net.module.cuda()

    with open(os.path.join(config.out_dir, "config.pickle"), 'wb') as outfile: pickle.dump(config, outfile)

    with open(os.path.join(config.out_dir, "config.txt"), "w") as text_file: text_file.write("%s" % config)
